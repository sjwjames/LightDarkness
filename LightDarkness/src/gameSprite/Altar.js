...
var Altar=cc.PhysicsSprite.extend({
    ...
    initPhysics:function (space) {
        ...
        var spotRadius=this.width/2*0.8;
        var body=new cp.Body(Number.MAX_VALUE, cp.momentForCircle(Number.MAX_VALUE,0,spotRadius,cp.v(0,0)));
        body.setPos(cc.p(pos_x,pos_y));
        space.addBody(body);
        var shape=new cp.CircleShape(body,spotRadius,cp.v(0,0));
